	...
	public function getCommentAttachments($id,$class = 'comment-single-attachment') {
		...
		if(!empty($thumbnails)) {
			foreach($thumbnails as $thumbnail) {
				...
				if(strpos($thumbnail, '.pdf')) $thumbnailsSend .= '<span class="right '.$class.' comment-pdf-attachment"><a href="' . str_replace('thumbnail-','',$thumbnail) .'" target="_blank" rel="gallery-'.$id.'"><img src="/images/pdficon.png" alt="'.$attachmentTitle.'"><span>'.$attachmentTitle.'</span></a></span>';
				else $thumbnailsSend .= '<span class="right '.$class.'"><a href="'. str_replace('thumbnail-','',$thumbnail) .'" rel="gallery-'.$id.'">'. HTML::image($thumbnail, $attachmentTitle, array('class' => 'comment-attachment')).'</a></span>';
			}
		}
